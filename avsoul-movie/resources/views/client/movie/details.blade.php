...
@section('main')
	...
	<section class="section section--head section--head-fixed section--gradient section--details-bg">
		<div class="section__bg" data-bg="{{$movie->images}}"></div>
		<div class="container">
			<!-- article -->
			<div class="article">
				<div class="row">
					<div class="col-12 col-xl-8">
						...
						<div class="article__content">
							...
							<ul class="list">
								...
								<li>{{$movie->country}}</li>
								<li>{{$movie->year}}</li>
								<li>{{ \Carbon\CarbonInterval::minutes($movie->duration)->cascade()->format('%h h %i min') }}</li>
								<li>{{$movie->age}}+</li>
							</ul>
					...
					<div class="col-12 col-xl-8">
						...
						<div class="categories">
                            ...
                            <a href="#" class="categories__item">{{$genre->title}}</a>
                            @endforeach
						</div>
						<!-- end categories -->
							...
							<div class="tab-content">
								...
								<div class="tab-pane fade show active" id="tab-1" role="tabpanel">
									<ul class="comments__list">
                                        @foreach ($comment_reply as $cmt_item)
										<li class="comments__item">
											<div class="comments__autor">
												<img class="comments__avatar" src="{{$cmt_item->user->avatar}}" alt="">
												<span class="comments__name">{{$cmt_item->user->name}}</span>
												<span class="comments__time">{{ \Carbon\Carbon::parse($cmt_item->created_at)->format('F j, Y, H:i') ?? '' }}</span>
											</div>
											<p class="comments__text">{{$cmt_item->content}}</p>
											<div class="comments__actions">
												<div class="comments__rate">
													<button type="button"><svg width="22" height="22"
														...
														</svg> {{$cmt_item->like_count}}</button>

													<button type="button">{{$cmt_item->dislike_count}} <svg width="22" height="22"
															viewBox="0 0 22 22" fill="none"
															xmlns="http://www.w3.org/2000/svg">
															<path d="M14.6667 10.9905H7.33333" stroke-width="1.5"
																stroke-linecap="round" stroke-linejoin="round"></path>
...
{{--
                                    ...
                                    {{ $comment_reply->links('client.widgets.paginator') }}
									...
									<form action="{{route('mov.comment')}}" method="POST" class="comments__form">
                                        <input type="hidden" name="movie_id" value="{{$movie->id}}">
										<div class="sign__group">
											<textarea id="text" name="content" class="sign__textarea"
									...
									<ul class="reviews__list">
										...
										<li class="reviews__item">
											<div class="reviews__autor">
												<img class="reviews__avatar" src="{{$rating->user->avatar ?? ''}}" alt="">
												<span class="reviews__name">{{$rating->title}}</span>
												<span class="reviews__time">{{ \Carbon\Carbon::parse($rating->created_at)->format('F j, Y, H:i') ?? '' }} by {{$rating->user->name ?? ''}}</span>
												<span class="reviews__rating"><svg xmlns="http://www.w3.org/2000/svg"
														viewBox="0 0 24 24">
														<path
															d="M22,9.67A1,1,0,0,0,21.14,9l-5.69-.83L12.9,3a1,1,0,0,0-1.8,0L8.55,8.16,2.86,9a1,1,0,0,0-.81.68,1,1,0,0,0,.25,1l4.13,4-1,5.68A1,1,0,0,0,6.9,21.44L12,18.77l5.1,2.67a.93.93,0,0,0,.46.12,1,1,0,0,0,.59-.19,1,1,0,0,0,.4-1l-1-5.68,4.13-4A1,1,0,0,0,22,9.67Zm-6.15,4a1,1,0,0,0-.29.88l.72,4.2-3.76-2a1.06,1.06,0,0,0-.94,0l-3.76,2,.72-4.2a1,1,0,0,0-.29-.88l-3-3,4.21-.61a1,1,0,0,0,.76-.55L12,5.7l1.88,3.82a1,1,0,0,0,.76.55l4.21.61Z" />
													</svg> {{$rating->rate}}</span>
											</div>
											<p class="reviews__text">{{$rating->content}}</p>
										</li>
                                        @endforeach
									</ul>
                                    {{ $ratings->links('client.widgets.paginator') }}

                                    @if ($flag == true)
                                        <div class="categories" style="text-align: center">
                                            <h1 class="categories__title">You have rated this movie</h1>
                                        </div>
                                    @else
                                    <form action="{{route('mov.review')}}" method="POST" class="reviews__form">
                                        <input type="hidden" name="movie_id" value="{{$movie->id}}">
										<div class="row">
											...
											<div class="col-12">
												<div class="sign__group">
													<textarea id="text2" name="content" class="sign__textarea"
